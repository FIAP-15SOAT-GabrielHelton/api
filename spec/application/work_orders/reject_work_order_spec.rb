...
require_relative "../../../app/application/work_orders/reject_work_order"

describe WorkOrders::RejectWorkOrder do
  let(:work_order) do
  ...
  describe "#call" do
    it "transitions awaiting_approval → rejected" do
    ...
    it "returns failure when reject is not allowed" do
      allow(repository).to receive(:find).with(1).and_return(
        WorkOrders::WorkOrder.new(
          id: 1, customer_id: 10, vehicle_id: 20, problem_description: "x", status: :in_progress
        )
      )
