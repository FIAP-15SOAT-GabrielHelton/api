...
RSpec.describe WorkOrders::CalculateAdminMetrics do
  ...
  it "returns the values from the read model" do
    allow(read_model).to receive_messages(average_execution_time_minutes: 45.5, completed_count: 7)

    result = use_case.call

    expect(result).to be_success
    expect(result.value).to eq(average_execution_time_minutes: 45.5, completed_count: 7)
  end
end
